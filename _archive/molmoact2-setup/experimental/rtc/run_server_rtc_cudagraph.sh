#!/usr/bin/env bash
# EXPERIMENTAL: cuda-graph variant. Stable launcher is run_server_rtc.sh.
# Starts host_server_rtc_cudagraph.py on port 8203 (same port). You MUST
# stop the stable server before launching this one.
# The first /act call against this server pays a one-time torch.compile
# tracing cost (~10-30 s); subsequent calls should be materially faster.
...
exec "$VENV/bin/python" "$SCRIPT_DIR/host_server_rtc_cudagraph.py" \
    --host 0.0.0.0 \
    --port 8203 \
    --dtype bfloat16 \
    "$@"
